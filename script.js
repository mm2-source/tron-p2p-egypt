...
function setAdMode(mode) {
    ...
    // Create Ad: quantity section controls (Max + balance text)
    const maxAmountBtn = document.getElementById('maxAmountBtn');
    const sellModeBalance = document.getElementById('sellModeBalance');
    ...
    if (mode === 'buy') {
        if (maxAmountBtn) {
            maxAmountBtn.style.display = 'none';
            maxAmountBtn.disabled = true;
            maxAmountBtn.style.pointerEvents = 'none';
        }
        if (sellModeBalance) sellModeBalance.style.display = 'none';
        if (publishBtn) {
        ...
        if (maxAmountBtn) {
            maxAmountBtn.style.display = 'inline-flex';
            maxAmountBtn.disabled = false;
            maxAmountBtn.style.pointerEvents = 'auto';
        }
        if (sellModeBalance) sellModeBalance.style.display = 'block';
        updateSellBalance();
        if (publishBtn) {
...
function updateCreateAdUIState() {
    ...
    if (!priceIn || !amountIn || !minIn || !maxIn || !publishBtn || !totalEl || !selectedPaymentText) return;

    const price = parseFloat(priceIn.value) || 0;
    const qty = parseFloat(amountIn.value) || 0;
    const min = parseFloat(minIn.value) || 0;
    const max = parseFloat(maxIn.value) || 0;
    const paymentMethod = selectedPaymentText.textContent?.trim() || '';

    const total = price > 0 && qty > 0 ? price * qty : 0;
    totalEl.textContent = total.toFixed(2);

    const hasPayment = paymentMethod !== '' && paymentMethod !== 'اختر طريقة الدفع' && paymentMethod !== 'Select payment method';
    const requiredFilled = price > 0 && qty > 0 && min > 0 && max > 0 && hasPayment && min <= max;
...
function handleCreateAdSubmit(btn) {
    if (!btn) return;
    if (btn.disabled) return;

    // Ensure state is up to date before submitting
    updateCreateAdUIState();
    if (btn.disabled) return;

    const originalHTML = btn.innerHTML;
    btn.disabled = true;
    btn.innerHTML = `<i class="fa-solid fa-spinner fa-spin ml-2"></i> جارٍ النشر...`;

    setTimeout(() => {
        // After exactly 20 seconds, proceed with the existing publish flow
...
async function handlePublishAd(passedBtn) {
    if(!tronWeb) return showToast("Connect your wallet first", "error");
        
    // Anti-double click protection
    const publishBtn = passedBtn || (typeof event !== 'undefined' && event && event.target) || document.getElementById('publishButton');
    ...
    try {
        ...
        if(adFormMode === 'buy') {
            ...
            if (!amount || amount <= 0) {
                ...
                publishBtn.innerHTML = 'Publish Ad';
                return;
            }
