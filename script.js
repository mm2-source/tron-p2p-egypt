...
function setAdMode(mode) {
    ...
    if(mode === 'buy') {
        ...
        // Hide balance and Max button for Buy mode
        ...
        // Show balance and Max button for Sell mode
...
// Balance Guard for Sell Mode Only
async function handleBalanceCheck(input) {
    ...
    try {
        ...
        if (adFormMode === 'buy') {
            // Buy mode - COMPLETELY DISABLE USDT balance validation
            // Always keep neutral black border, no red borders ever
            container.className = 'bg-gray-50 p-2 rounded-lg border border-black';
            return; // Exit early for buy mode - NO validation
        }
        
        // Sell mode logic only
        const amountValue = parseFloat(input.value) || 0;
        if (balance === 0.00) {
            // Zero balance - prevent publishing
            input.value = '0.00';
            container.className = 'bg-gray-50 p-2 rounded-lg border border-red-500';
            ...
            container.className = 'bg-gray-50 p-2 rounded-lg border border-green-500';
        } else {
            // Default black border for empty input
            container.className = 'bg-gray-50 p-2 rounded-lg border border-black';
        }
    } catch (error) {
        console.error('Error checking balance:', error);
...
async function setMaxAmount() {
    ...
    try {
        ...
        // Trigger balance check to update validation
        handleBalanceCheck(amountInput);
        
        showToast(`Set to maximum balance: ${balance.toFixed(2)} USDT`, 'success');
    } catch (error) {
