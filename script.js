...
function switchInputTab(tab) {
    ...
    const maxQuantityBtn = document.getElementById('maxQuantityBtn');
...
function setAdMode(mode) {
    ...
    if(mode === 'buy') {
        // Buy mode - Green active state (right side)
        b.className = "flex-1 py-3 rounded-xl font-bold bg-green-600 text-white shadow-sm";
        s.className = "flex-1 py-3 rounded-xl font-bold text-gray-400";
        // Hide balance for Buy mode (buyers don't need to see their USDT balance)
        balanceDiv.classList.add('hidden');
        balanceDiv.style.display = 'none';
        // Green button for Buy mode
        ...
        // Sell mode - Red active state (left side)
        s.className = "flex-1 py-3 rounded-xl font-bold bg-red-600 text-white shadow-sm";
        b.className = "flex-1 py-3 rounded-xl font-bold text-gray-400";
        // Show balance for Sell mode (sellers need to see their USDT balance)
        balanceDiv.classList.remove('hidden');
        balanceDiv.style.display = 'block';
        updateSellBalance();
        // Red button for Sell mode
...
// Balance Guard for Both Buy and Sell Modes
async function handleBalanceCheck(input) {
    ...
    try {
        ...
        const container = input.closest('.bg-gray-50'); // Get the parent container
        
        if (adFormMode === 'buy') {
            // Buy mode logic - prevent typing if balance is 0.00
            if (balance === 0.00) {
                input.value = '0.00';
                // Red border for container
                container.className = 'bg-gray-50 p-4 rounded-2xl border border-red-500';
                // Show toast notification for insufficient balance
                showToast('Your balance is insufficient. You must have USDT balance to create a buy ad.', 'error');
            } else {
                // Green border for sufficient balance
                container.className = 'bg-gray-50 p-4 rounded-2xl border border-green-500';
            }
        } else if (adFormMode === 'sell') {
            // Sell mode logic - show red border if trying to sell more than available
            const amountValue = parseFloat(input.value) || 0;
            if (amountValue > balance) {
                // Red border for insufficient balance
                container.className = 'bg-gray-50 p-4 rounded-2xl border border-red-500';
                showToast('Insufficient balance. You cannot sell more than your available USDT.', 'error');
            } else if (amountValue > 0) {
                // Green border for valid amount
                container.className = 'bg-gray-50 p-4 rounded-2xl border border-green-500';
            } else {
                // Default black border
                container.className = 'bg-gray-50 p-4 rounded-2xl border border-black';
            }
        }
    } catch (error) {
        console.error('Error checking balance:', error);
...
async function handlePublishAd() {
    if(!tronWeb) return showToast("Connect your wallet first", "error");
    ...
    try {
        ...
        if(!price) {
            throw new Error('Please enter exchange rate');
        }
        if(!amount || parseFloat(amount) <= 0) {
            throw new Error('Please enter a valid quantity greater than 0');
        }
        if(!min || parseFloat(min) <= 0) {
            throw new Error('Please enter a valid minimum greater than 0');
        }
        if(!max || parseFloat(max) <= 0) {
            throw new Error('Please enter a valid maximum greater than 0');
        }
        ...
        if(maxLimitValue > totalValue) {
            throw new Error(`Maximum limit (${max} EGP) exceeds total ad value (${totalValue.toFixed(2)} EGP). Please correct the maximum limit.`);
        }
        if(!paymentMethod || paymentMethod === 'Select payment method' || paymentMethod.trim() === '') {
            throw new Error('Please select a payment method - this field is mandatory');
        }
        ...
        if(adFormMode === 'buy') {
            try {
                ...
                if(balance < parseFloat(amount)) {
                    throw new Error("Sorry, your wallet balance is less than the quantity specified for the ad.");
                }
                    
                if (!confirm(`Will be locked ${amount} USDT in the smart contract`)) {
                ...
                showToast("Wallet verification failed: " + e.message, "error"); 
                publishBtn.disabled = false;
                publishBtn.textContent = 'Publish Ad';
                return;
            }
        ...
        showToast('Error publishing ad: ' + error.message, 'error');
        publishBtn.disabled = false;
        publishBtn.textContent = 'Publish Ad';
    }
}
