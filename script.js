...
// My Ads is a control panel for ACTIVE ads only (OKX-style).
...
async function publishAd() {
  ...
  try {
    ...
    // Bridge: Publication -> 20s Animation -> Redirect to "My Ads" -> Display active card.
    navTo("ads");
  } catch (e) {
    console.error(e);
...
// ---- My Ads (Clean Rewrite) ----
let unsubMyAds = null;

function subscribeMyAds() {
  // Why: render a professional control panel for the merchant's ACTIVE ads.
  if (!db) return;
  if (!connectedAddress) return;
  ...
  const countEl = document.getElementById("myAdsActiveCount");
  if (!list) return;
  ...
  unsubMyAds = db
    ...
    .orderBy("timestamp", "desc")
    .onSnapshot((snap) => {
      if (countEl) countEl.textContent = String(snap.size || 0);
          ...
          return `
            <article class="adCard">
              <div class="adCard__top">
                ...
                <div class="meta"><b>نشط</b></div>
              </div>

              <div class="meta">النوع: <b>${typeLabel}</b></div>
              ...
              <div class="priceRow" style="margin-top:12px;">
                ...
                <div style="display:flex; gap:10px;">
                  <button class="actionBtn actionBtn--green" type="button" onclick="editMyAd('${d.id}')">تعديل</button>
                  <button class="actionBtn actionBtn--red" type="button" onclick="cancelMyAd('${d.id}')">إلغاء</button>
                </div>
              </div>
            </article>
          `;
...
async function cancelMyAd(adId) {
  // Why: merchant cancels/removes an ad from Firebase.
  if (!db) return;
