...
let headerBalanceInterval = null;
let currentPageKey = "p2p";
...
async function refreshHeaderBalanceUI() {
  ...
  if (!wrap || !textEl || !iconEl) return;

  // Only show on certain pages
  if (!["p2p", "orders", "chat", "profile"].includes(currentPageKey)) return;

  let bal = 0;
  if (connectedAddress) bal = await getUSDTBalance(connectedAddress);

  textEl.textContent = `رصيدك: ${format2(bal)} USDT`;
...
function openCreateAdFromMyAds() {
  // Why: bridge button from My Ads empty state to create form.
  openCreateAd();
}
...
async function publishAd() {
  ...
  try {
    ...
    if (editingId) {
      // Keep ad active after edit; preserve timestamps by only updating fields.
    ...
    // Bridge: after successful publish, show "My Ads" (Active tab) automatically.
    myAdsTab = "active";
    navTo("ads");
  } catch (e) {
    console.error(e);
...
function updateHeaderForPageKey(page) {
  currentPageKey = page;
  ...
  const showPlus = page === "ads";
  // Balance on P2P / Orders / Chat / Profile
  const showBalance = ["p2p", "orders", "chat", "profile"].includes(page);

  plusBtn.style.display = showPlus ? "inline-flex" : "none";
  bal.style.display = showBalance ? "inline-flex" : "none";

  // Keep header balance fresh
  if (showBalance) refreshHeaderBalanceUI();
}
...
function subscribeMyAds() {
  ...
  unsubMyAds = db
    ...
    .onSnapshot((snap) => {
      ...
      const filtered = docs.filter((a) => (myAdsTab === "active" ? a.status === "active" : a.status !== "active"));

      // Empty state (match requested copy)
      ...
      list.innerHTML = filtered
        .map((a) => {
          const typeLabel = a.type === "buy" ? "شراء" : "بيع";
          const statusLabel = a.status === "active" ? "نشط" : "غير نشط";
          const toggleText = a.status === "active" ? "إيقاف" : "تفعيل";
          const toggleClass = a.status === "active" ? "actionBtn actionBtn--red" : "actionBtn actionBtn--green";
          ...
          return `
            <article class="adCard">
              <div class="adCard__top">
                ...
                <div class="meta"><b>${statusLabel}</b></div>
              </div>

              <div class="meta">النوع: <b>${typeLabel}</b> • المتاح: <b>${format2(a.availableQuantity)}</b> USDT</div>
              ...
              <div class="priceRow" style="margin-top:12px;">
                ...
                <div style="display:flex; gap:10px;">
                  <button class="icon-btn" type="button" onclick="editMyAd('${a.id}')" aria-label="تعديل"><i class="fa-solid fa-pen"></i></button>
                  <button class="${toggleClass}" type="button" onclick="toggleAdStatus('${a.id}', '${a.status || "active"}')">${toggleText}</button>
                </div>
              </div>
            </article>
          `;
...
async function toggleAdStatus(adId, currentStatus) {
  ...
  await db.collection("Ads").doc(adId).update({ status: next });
  toast(next === "active" ? "تم تفعيل الإعلان" : "تم إيقاف الإعلان");
}

async function editMyAd(adId) {
  // Why: open create form pre-filled to edit an ad.
  if (!db) return;
  const doc = await db.collection("Ads").doc(adId).get();
  if (!doc.exists) return toast("الإعلان غير موجود");
  const a = doc.data() || {};
  ...
  const selectedPaymentText = document.getElementById("selectedPaymentText");
  if (selectedPaymentText) selectedPaymentText.textContent = a.paymentMethod || "اختر طريقة الدفع";

  // Mode inferred from ad.type
  await setCreateMode(a.type === "sell" ? "sell" : "buy");

  // Mark edit state on publish button
  const publishBtn = document.getElementById("publishBtn");
...
window.openCreateAdFromMyAds = openCreateAdFromMyAds;
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Periodic header balance refresh (when wallet is connected)
  if (headerBalanceInterval) clearInterval(headerBalanceInterval);
  headerBalanceInterval = setInterval(() => {
    if (connectedAddress) refreshHeaderBalanceUI();
  }, 15000);
});
