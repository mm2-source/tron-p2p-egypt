...
function setAdMode(mode) {
    ...
    if(mode === 'buy') {
        ...
        // COMPLETELY HIDE Max button and Available text in Buy mode
        ...
        // No toast message needed for buy mode
...
async function handleBalanceCheck(input) {
    if (adFormMode === 'buy') {
        // BUY MODE: ZERO RESTRICTIONS - NO VALIDATION
        // Allow any amount without any checks
        const container = input.closest('.bg-gray-50');
        if (container) {
...
async function handlePublishAd() {
    ...
    try {
        ...
        // [Security Operations]: NO balance validation for Buy mode - allow any amount
        if(adFormMode === 'buy') {
            // Buy mode: Direct publish without any balance validation
            await saveAdToDatabase(publishBtn, paymentMethod);
        } else {
            // Sell mode: Check balance before publishing
            const amount = parseFloat(document.getElementById('amountIn').value);
            if (!amount || amount <= 0) {
                showToast('Please enter a valid amount', 'error');
                publishBtn.disabled = false;
                publishBtn.innerHTML = 'نشر الإعلان';
                return;
            }
            
            const balance = await checkUSDTBalance(myAddr);
            if (amount > balance) {
                showToast(`Insufficient balance. Available: ${balance.toFixed(2)} USDT`, 'error');
                publishBtn.disabled = false;
                publishBtn.innerHTML = 'نشر الإعلان';
                return;
            }
